...
%   1.0 (aug 14, 2006) - update for paper
   ...
   global tumin radiusearthkm xke j2 j3 j4 j3oj2  
    Tb=0.0001%(7.96500000000000e+04)/60;
    Valmainrv=[];
    Valmaincoe=[];
% // ------------------------  implementation   --------------------------
...
%         // input 2-line element set file
    infilename ='inspire.txt'%'sample.txt'%'output2f.out'% input('input elset filename: ','s');
...
%                            tsince,ro(1),ro(2),ro(3),vo(1),vo(2),vo(3));
    ...
    fclose(outfile);
figure(2)
plot3(Valmainrv(:,1),Valmainrv(:,2),Valmainrv(:,3),'ro')